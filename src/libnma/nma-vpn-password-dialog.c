...
{
	NMAVpnPasswordDialogPrivate *priv;
	gs_free char *text_free = NULL;
	char *text = _("Request expired");
	...
	if (reason)
		text = text_free = g_strdup_printf ("%s : %s", text, reason);
	gtk_label_set_text (GTK_LABEL (priv->warning_label), text);
}
